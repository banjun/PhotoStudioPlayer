...
class ChromaKeyFilter {
    static func filter(_ targetRed: Float, green targetGreen: Float, blue targetBlue: Float, threshold: Float) -> CIFilter {
        let size = 64
        let cubeData = malloc(size * size * size * MemoryLayout<Float>.size * 4)
        var c = UnsafeMutablePointer<Float>(OpaquePointer(cubeData))
        ...
        let data = NSData(bytesNoCopy: cubeData!, length: size * size * size * MemoryLayout<Float>.size * 4, freeWhenDone: false)
